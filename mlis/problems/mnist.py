...
class SolutionModel(nn.Module):
    def __init__(self, input_size, output_size, solution):
        ...
        self.bn1   = torch.nn.BatchNorm1d(40, track_running_stats=False)
        
        self.fc2   = torch.nn.Linear(40, 30)
        self.act4  = torch.nn.ReLU()
        self.bn2   = torch.nn.BatchNorm1d(30, track_running_stats=False)
        
        self.fc3   = torch.nn.Linear(30, output_size)
...
class Solution():
    def __init__(self):
        ...
        self.hidden_size = 16
        self.n_hidden2 = 1
        self.hidden_size_2 = 40

        self.threshold_error = 0.3

        self.momentum = 0.8673469387755102

        self.activation_hidden = nn.ReLU6()
        ...
        self.lr_grid = np.linspace(0.5, 1.5, 11)
        
        self.momentum_grid = list(np.linspace(0.5, 1, 6))
    ...
    def train_model(self, train_data, train_target, context):
        ...
        model = SolutionModel(train_data.size(1), len(train_target.unique()), self)
        step = 0
        model.train()
        if self.weight_init:
